...
# 11 "secure_super_aggressive_stack_checking.h"
void* protected_arrays[100];

void ___protect_arrays(int numArgs, ...){
 va_list args;
 __builtin_va_start(args,numArgs);
 for (int j = 0; j < numArgs && j < 100; j++){
  unsigned addr = __builtin_va_arg(args,int);
...
# 49 "secure_super_aggressive_stack_checking.h"
inline void sprintf_check_setup(char *dest, const char *fmt, ...) {
 ...
 if(nextHighestAddress != 0x0){
  if(valueAtNextHighestAddress != nHAPtr[0])
   __asm__ __volatile__( "CALL __stack_chk_fail\n" );
 }
...
int a(){
    ...
    char buff[10];
    char buff2[10] = "dir";
    void* _x[] = { buff, buff2 }; ___protect_arrays(sizeof(_x)/sizeof(_x[0]), buff, buff2);
...
# 48 "secure_super_aggressive_stack_checking.h"
