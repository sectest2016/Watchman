...
{
 srand(time(__null));
 canary = rand() % (2147483647 * 2U + 1U) + 1;
...
___Canary :: ___Canary(unsigned int check) : test(__null), canary(check)
{
...
void __cyg_profile_func_enter(void *this_fn, void *call_site) {
 ...
 if(numCanaries != 10000 && canaryOverflow == 0){
  canaries[numCanaries] = new ___Canary();
    numCanaries++;
 }
 else{
